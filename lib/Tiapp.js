...
Tiapp.prototype.incrementVersion = function incrementVersion(sem) {
	var current, matches, incremented, androids, manifests, manifest;

	// make sure sem is a string and one of major, minor or patch
	if (!sem || !U.isString(sem) || ['major', 'minor', 'patch'].indexOf(sem) === -1) {
	...
	if (sem === 'major') {
		incremented = (parseInt(matches[1], 10) + 1).toString() + '.0.0';
	} else if (sem === 'minor') {
		incremented = matches[1] + '.' + (parseInt(matches[2], 10) + 1).toString() + '.0';
	} else {
		incremented = matches[1] + '.' + matches[2] + '.' + (parseInt(matches[3], 10) + 1).toString();
	}
