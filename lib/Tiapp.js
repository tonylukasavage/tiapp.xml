...
Tiapp.prototype.incrementVersion = function incrementVersion(sem) {
	var current, matches, index, incremented, androids, manifests, manifest;

	// make sure sem is a string and one of major, minor or patch
	if (!sem || !U.isString(sem) || (index = ['major', 'minor', 'patch'].indexOf(sem) + 1) === 0) {
		throw new Error('Bad argument. sem must be one of `major`, `minor` or `patch`.');
	}

	current = this.version;

	// make sure current is a valid string holding a sem version
	if (!current || !U.isString(current) || !(matches = current.match(/^([0-9]+)\.([0-9]+)\.([0-9]+)$/))) {
		throw new Error('Bad value. current version must be a semantic version.');
	}

	// increment the major/minor/patch version in the right match
	matches[index] = (parseInt(matches[index], 10) + 1).toString();

	// join the matches to get the full incremented version
	incremented = matches.splice(1, 3).join('.');

	this.version = incremented;
	...
	if (androids.length === 1) {
		...
		if (manifests.length === 1) {
			...
			// remove versionName - ti build will use the top-level version
			if (manifest.hasAttribute('android:versionName')) {
				manifest.removeAttribute('android:versionName');
			}
